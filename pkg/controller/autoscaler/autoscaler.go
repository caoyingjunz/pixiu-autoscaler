...
func (ac *AutoscalerController) updateDeployment(old, current interface{}) {
	...
	if oldOk && curOk {
		if oldMaxReplicas != curMaxReplicas {
			...
			if err != nil {
				// HPA 不存在
				curHpa := controller.CreateHorizontalPodAutoscaler(curD.Name, curD.Namespace, curD.UID, curD.APIVersion, curD.Kind, int32(maxReplicasInt))
				ac.client.AutoscalingV2beta2().
			...
			ac.client.AutoscalingV2beta2().HorizontalPodAutoscalers(curD.Namespace).Update(context.TODO(), hpa, metav1.UpdateOptions{})
		}
	} else if oldOk && !curOk {
		// DELETE HPA
		...
		hpa := controller.CreateHorizontalPodAutoscaler(curD.Name, curD.Namespace, curD.UID, curD.APIVersion, curD.Kind, int32(maxReplicasInt))
		ac.client.AutoscalingV2beta2().
			HorizontalPodAutoscalers(curD.Namespace).
