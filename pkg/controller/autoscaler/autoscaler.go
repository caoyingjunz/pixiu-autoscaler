...
func (ac *AutoscalerController) handerHPAEvent(hpa *autoscalingv2.HorizontalPodAutoscaler) error {
	...
	newHpa := controller.CreateHorizontalPodAutoscaler(hpa.Name, hpa.Namespace, uid, apiVersion, hpa.Spec.ScaleTargetRef.Kind, int32(maxReplicasInt))
	_, err = ac.client.AutoscalingV2beta2().HorizontalPodAutoscalers(hpa.Namespace).Create(context.TODO(), newHpa, metav1.CreateOptions{})
	if err != nil {
